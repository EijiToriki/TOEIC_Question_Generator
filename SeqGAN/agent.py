...
class Agent(object):
    def __init__(self, sess, vocab_size, emb_size, hidden_size,
        ...
        #コンストラクタでは
        #generatorとrollouter同じ
        #Actor→行動器（エージェントに含まれる）
    ...
    def _build_pre_generator(self, vocab_size, emb_size, hidden_size):
        ...
        #LSTM層
        #第二引数：return_sequences=True 毎時刻の出力を得る
        out = LSTM(hidden_size, return_sequences=True, name='LSTM')(out)

        #TimeDistributed→入力のすべての時間スライスにレイヤーを適用可能
        out = TimeDistributed(
            #全結合層
    ...
    def pre_train(self, g_data, g_pre_episodes, weight_path, g_pre_lr):
        #Adam:最適アルゴリズムの1つ．モデルをコンパイルする際に必要
        g_optimizer = Adam(g_pre_lr)
        #コンパイル：'categorical_crossentropy'はモデルが最小化する目的関数
...
class Actor(object):
    ...
    def _build_graph(self):
        #プレースホルダ：データが格納される入れ物
        #               データは未定のままグラフを構築
        #               具体的な値を実行時に与えられる
        state_in = tf.compat.v1.placeholder(tf.float32, shape=(None, 1))
        h_in = tf.compat.v1.placeholder(tf.float32, shape=(None, self.hidden_size))
        c_in = tf.compat.v1.placeholder(tf.float32, shape=(None, self.hidden_size))
        action = tf.compat.v1.placeholder(tf.float32, shape=(None, self.vocab_size))
        reward = tf.compat.v1.placeholder(tf.float32, shape=(None, 1))
        ...
        #log(x)：底がeのlog
        #reduce_sum()：すべての要素で和をとりスカラーで返す
        #               axis=-1により1つのリストの和を返す
        # ex)reduce_sum([[[1, 2, 4], [1, 0, 3]],[[1,2,3],[2,2,1]]],axis=-1) → [[7,4],[6,5]]
        log_prob = tf.math.log(tf.reduce_sum(prob * action, axis=-1))
        loss = - log_prob * reward
        #tf.train.AdamOptimizer → Adamアルゴリズムを実装するオプティマイザー
        #Adamアルゴリズム → もっともよく使われている最適化アルゴリズム
        #オプティマイザー → 速くて楽になるように調整してくれる機能　
